...
if (( "$#" < 1 )); then
    ...
    echo
    echo "Usage: 
        $0 <schemaRegistryUrl> [Optional: Beginning Schema ID, Default: 10,000]
        
    Example: 
        $0 http://localhost:8081 500
    "
    exit
fi
...
RESULT_SUBJECTS=$(curl --silent -k $SRURL"/schemas")
if [ "$RESULT_SUBJECTS" != "[]" ]
then
...
RESULT_SETIMPORT=$(curl --silent -k -X PUT -d "$IMPORT_PAYLOAD" -H "$CONTENT" $SUBJECTMODE_URL)

## Register dummy schema
RESULT_REGISTER=$(curl --silent -k -X POST -d "$DUMMYSCHEMA_IMPORT" -H "$CONTENT" $SUBJECTSCHEMA_ENDPOINT)

## Set READWRITE on a single subject
RESULT_SETRW=$(curl --silent -k -X PUT -d "$READWRITE_PAYLOAD" -H "$CONTENT" $SUBJECTMODE_URL)

## Soft delete dummy schema to maintain the beginning ID
RESULT_DELETE=$(curl --silent -k -X DELETE $SUBJECTSCHEMA_ENDPOINT"/1")
HOLD=$(curl --silent -k -X DELETE $SUBJECTSCHEMA_ENDPOINT"/1?permanent=true")

## Register another dummy and confirm ID=SRBEGINNING+1 <- Sanity Check
OUTPUT=$(curl --silent -k -X POST -d "$DUMMY_SCHEMA" -H "$CONTENT" $SUBJECTSCHEMA_ENDPOINT_2)
...
## Cleanup
HOLD=$(curl --silent -k -X DELETE $SUBJECTSCHEMA_ENDPOINT_2"/1")
HOLD=$(curl --silent -k -X DELETE $SUBJECTSCHEMA_ENDPOINT_2"/1?permanent=true")
